...
from chess_zero.env.chess_env import GoBangEnv, Winner, PANEL_SIZE, pretty_print_panel, get_state_by_input_planes
...
class ChessPlayer:
    ...
    def action(self, env: GoBangEnv, can_stop = True) -> (str):
        ...
        #     return None
        # else:
        self.moves.append([get_state_by_input_planes(env.observation), list(policy)]) # ??list??????json.dump()????
        # ?????????
        move = [env.observation, list(policy)]
        for _state, _policy in Config.flip_moves(move):
            self.moves.append([get_state_by_input_planes(np.array(_state)), _policy])
    ...
    def search_moves(self, env: GoBangEnv) -> (float, float):
        ...
        :param GoBangEnv env: env to search for moves within
    ...
    def search_my_move(self, env: GoBangEnv, is_root_node=False) -> (float):
        """
        ...
        :param GoBangEnv env: environment in which to search for the move
        ...
        # pretty_print_panel(state)
        # print(f'state:{state}, after action:{action_t}, esimated score: {leaf_v:.4}, and n,w,q are:{my_stats.n},{my_stats.w:.4},{my_stats.q:.4}')
        return leaf_v

    def expand_and_evaluate(self, env: GoBangEnv) -> (np.ndarray, float):
    ...
    def predict(self, state_planes):
        ...
        # import random
        # policy = np.array([random.random() for _ in range(self.labels_n)])
        # policy = policy / sum(policy)
        # ret = [policy, random.random()*2-1]
        return ret

    #@profile
    def select_action_q_and_u(self, env: GoBangEnv, is_root_node):
        """
